...
//	///< ƒл€ толщиномера необходима скорость звука в среде
...
//			//	Version::StoreScopeVelocity(i, data.scope_velocity[i]); ///< дл€ востановлени€(портитс€ при загрузке данных дл€ просмотра. ¬останавливать в начале цикла)
...
//	};
	...
	template<class O, class P>struct __do__
	{
		bool operator()(P &p)
		{
			if(__delay__<O, 10>::Do()) return true;///< выполн€тс€ будет примерно через каждые 50 мск
			if(Singleton<OnTheJobTable>::Instance().items.get<OnTheJob<O>>().value)
			{				
